...
class XOsintProvider(SearchProvider):
    ...
    async def search(
        ...
        # Direct OSINT query targeting public X (Twitter) posts, profiles, and threads
        clean_q = query.replace("site:x.com", "").replace("site:twitter.com", "").strip()
        encoded = quote(f"(site:x.com OR site:twitter.com) {clean_q}")
        ...
        try:
            ...
            for node in nodes:
                ...
                if a_tags:
                    ...
                    if (
                        target_url
                        and any(d in target_url for d in ("x.com", "twitter.com"))
                        and title
